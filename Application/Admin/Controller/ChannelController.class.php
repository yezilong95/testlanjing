...
class ChannelController extends BaseController
{
    ...
    public function saveEditAccount()
    {
        if (IS_POST) {
            ...
            $_request['all_money']      = $papiacc['all_money'];
            $_request['min_money']      = $papiacc['min_money'];
            $_request['max_money']      = $papiacc['max_money'];
    ...
    //一键将切换为该通道
    public function switchAllChannel(){
        $pid = I('get.pid');
        $paytype = M('channel as c')
            ->where(array("id"=>$pid))
            ->field('c.paytype')
            ->find();

        $channelId = M('product as p')
            ->where(['paytype'=>$paytype['paytype']])
            ->field('p.id')
            ->find();

        $productUser = M('product_user as pu')
            ->where(['pid'=>$channelId['id'],'status'=>'1','_logic'=>'and'])
            ->save(["channel"=>$pid]);
        if($productUser){
            $this->success('修改成功','',2);
        }else{
            $this->error('没有符合条件的商户，即将返回列表页');
        }
    }
