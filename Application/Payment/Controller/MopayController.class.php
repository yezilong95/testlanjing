...
class MopayController extends PaymentController{
    ...
    public function PaymentExec($wttlList, $pfaList){
        ...
        $logTitle = time() . '-摩宝代付-';
        addSyslog($logTitle.'提交到通道的未加密字符串: '.arrayToXml($transBody));
        ...
        $dataStr = $this->arrayToJson($arraystr);
        addSyslog($logTitle.'提交到通道的加密字符串: '.$dataStr);
        $returnData = curlPost($pfaList['exec_gateway'], $dataStr, ['Content-Type: application/json; charset=gbk',]);

        $returnData = json_decode($returnData, true);
        $returnData['refMsg'] =  iconv('GBK', 'UTF-8',  urldecode($returnData['refMsg']));

        addSyslog($logTitle.'通道返回的加密字符串: '.arrayToXml($returnData));
        ...
        if($returnData['status'] == '00'){
            ...
            addSyslog($logTitle.'通道返回的解密字符串: '.arrayToXml($info));
            if($info['refCode'] == '00'){
