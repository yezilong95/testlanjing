...
class WxGzhController extends PayController
{
    ...
    public function Pay($array)
    {
       ...
       echo $url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" . $return["appid"] . "&redirect_uri=" . $redirect_uri . "&response_type=code&scope=snsapi_base&state=" . $state . "#wechat_redirect";
        //header("Location:" . $url);
        redirect($url);
        exit();
    }
    ...
    public function jsapi()
    {
        ...
        $channel = M('Channel')->where(['code'=>$return['pay_ytongdao']])->find();
        $urlObj["appid"] = $channel['appid'];
        $urlObj["secret"] = $channel['appsecret'];
        $urlObj["code"] = $code;
        ...
        //签名步骤二：在string后加入KEY
        $return["key"] = $this->getSignkey('WxGzh', $return["memberid"]); // 密钥
    ...
    public function notifyurl()
    {
        ...
        file_put_contents('jm.txt',$streamData);
        $arraystr = xmlToArray($streamData);
        $pkey = $this->getSignkey('WxGzh', $arraystr["mch_id"]); // 密钥
