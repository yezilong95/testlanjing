...
/**
 * 途贝-微信扫码支付
 * Class TuBeiWxScanController
 * @package Pay\Controller
 */
class TuBeiWxScanController extends PayController
{
    public function Pay($array)
    {
        ...
        $notifyurl = $this->_site . 'Pay_TuBeiWxScan_notifyurl.html'; //异步通知
        $callbackurl = $this->_site . 'Pay_TuBeiWxScan_callbackurl.html'; //跳转通知
        $parameter = array(
            'code' => 'TuBeiWxScan', // 通道名称
            'title' => '途贝-微信扫码', //通道名称
            'exchange' => 1, // 金额比例
            'gateway' => '',
            'orderid'=>'',
        ...
        if ($return) {
            ...
            $data = array(
                ...
                'trade_type'        => 'trade.weixin.native', //交易类型   trade.weixin.native 为微信H5支付，更多支付类型请查看技术文档编写
            );
            //签名
    ...
    public function notifyurl()
    {
        ...
        if ($data['return_code'] == 'SUCCESS' && $data['result_code'] == 'SUCCESS' && $respSign == $sign) {
            ...
            $this->EditMoney($data["out_trade_no"], 'TuBeiWxScan', 0);
            exit('success');
        } else {
            exit('fail');
        }
